...
function CustomButton({ children, type="button", btn_bg, btn_border, btn_no_bg, btn_ter, handleClick=f=>f, isDisabled, ids="" }) {
  console.log(ids);
  // Custom Classes
  ...
  return (
    <button type={ type } className={`button ${customClasses}`} onClick={ handleClick } aria-disabled={isDisabled} disabled={isDisabled} id={`${ids}`} >
        { children }
    </button>
  )
}
